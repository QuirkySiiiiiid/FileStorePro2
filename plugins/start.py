...
back_button = InlineKeyboardButton("🔙 Back", callback_data="back_start")
...
async def start_command(client: Client, message: Message):
    ...
    # Delete previous /start messages from this user
    try:
        async for msg in client.search_messages(message.chat.id, query="/start"):
            await msg.delete()
    except Exception:
        pass
    ...
    start_msg = await message.reply_photo(
        ...
        caption=START_MSG.format(first=message.from_user.mention),
...
async def callback_handlers(client: Client, callback: CallbackQuery):
    """Handle all callback queries"""
    chat_id = callback.message.chat.id
    message_id = callback.message.id
    data = callback.data
    
    # Get random image for each update
    ...
    if data == "more":
        ...
        await callback.message.edit_media(
            media=InputMediaPhoto(
                media=random_pic,
                caption="Here are more options:",
            ),
            reply_markup=keyboard
        )

    elif data == "stats":
        # Dynamically get stats from database
        total_users = await db.total_users_count()
        total_banned = await db.total_banned_users()
        # Adjusted to dynamically get the number of active FSub channels
        fsub_channels = [x for x in FORCE_SUB_CHANNELS if x != 0]
        total_channels = len(fsub_channels)
        
        stats_text = f"""
»  Cᴏɴғɪɢᴜʀᴀᴛɪᴏɴs
» ᴛᴏᴛᴀʟ ғᴏʀᴄᴇ sᴜʙ ᴄʜᴀɴɴᴇʟ: {total_channels}
» ᴛᴏᴛᴀʟ ᴀᴅᴍɪɴs: {len(ADMINS)}
» ᴛᴏᴛᴀʟ ʙᴀɴɴᴇᴅ ᴜsᴇʀs: {total_banned}
» ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴍᴏᴅᴇ: {'Eɴᴀʙʟᴇᴅ' if TIME > 0 else 'Dɪsᴀʙʟᴇᴅ'}
» ᴘʀᴏᴛᴇᴄᴛ ᴄᴏɴᴛᴇɴᴛ: {'Eɴᴀʙʟᴇᴅ' if PROTECT_CONTENT else 'Dɪsᴀʙʟᴇᴅ'}
» ʜɪᴅᴇ ᴄᴀᴘᴛɪᴏɴ: Dɪsᴀʙʟᴇᴅ
» ᴄʜᴀɴɴᴇʟ ʙᴜᴛᴛᴏɴ: {'Eɴᴀʙʟᴇᴅ' if not DISABLE_CHANNEL_BUTTON else 'Dɪsᴀʙʟᴇᴅ'}
» ʀᴇǫᴜᴇsᴛ ғsᴜʙ ᴍᴏᴅᴇ: Eɴᴀʙʟᴇᴅ
"""
        keyboard = InlineKeyboardMarkup([[back_button, close_button]])
        await callback.message.edit_media(
            media=InputMediaPhoto(
                media=random_pic,
                caption=stats_text
        ...
        about_text = f"""
›› ᴍʏ ɴᴀᴍᴇ: **__{client.me.username}__**
›› ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ: {UPDATE_CHANNEL}
›› ᴏᴡɴᴇʀ: {OWNER}
›› ʟᴀɴɢᴜᴀɢᴇ: Pʏᴛʜᴏɴ 3
›› ʟɪʙʀᴀʀʏ: Pʏʀᴏɢʀᴀᴍ ᴠ2
›› ᴅᴀᴛᴀʙᴀsᴇ: Mᴏɴɢᴏ ᴅʙ
...
›› /stats : ᴠɪᴇᴡ sᴛᴀᴛɪsᴛɪᴄs"""
        ...
        await callback.message.edit_media(
            media=InputMediaPhoto(
                media=random_pic,
                caption=START_MSG.format(first=callback.from_user.mention)
            ),
            reply_markup=start_keyboard
        )
